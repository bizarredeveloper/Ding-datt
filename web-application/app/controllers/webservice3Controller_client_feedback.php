...
public function mobilelogin()
{
		...
		$userid =  ProfileModel::select('ID','password','status','username','firstname','lastname')->where('username',$user_name)->Orwhere('email',$user_name)->get();
	...
	//echo Carbon::now()->timezoneName;                            // America/Toronto
	 ...
	 $profile = ProfileModel::find($userid);
		...
		if($profile->status==1){
			...
			if($profile->profilepicture!=''){ $profile->profilepicture  =  url().'/public/assets/upload/profile/'.$profile->profilepicture;  }
...
public function getgroupdetails()
{
	...
	if($groubdetails[0]->groupimage!=''){ $groubdetails[0]->groupimage =url().'/public/assets/upload/group/'.$groubdetails[0]->groupimage; }
	$Response = array(
                'success' => '1',
...
public function getgrouplistforcontest()
{
	...
	//return count($grouplist);
		...
		$invited = invitegroupforcontestModel::where('group_id',$grouplist[$k]['groupid'])->where('contest_id',$contest_id)->get()->count();
		
		$groupmemberlist = groupmemberModel::where('group_id',$grouplist[$k]['groupid'])->get()->count();
		...
		{
		if($invited+2==$groupmemberlist && $groupmemberlist!=2 ){ $invitecnt = 1; }else if($invited!=0){ $invitecnt = -1;  }else{ $invitecnt = 0; }
		}else{
		if($invited+1==$groupmemberlist && $groupmemberlist!=1 ){ $invitecnt = 1;  }else if($invited!=0){ $invitecnt = -1;  }else{ $invitecnt = 0; }
...
//invitegroupsforcontest
	...
	if($invite_type=='single'){
		...
		{
		$invited=invitegroupforcontestModel::where('group_id',$group_id)->where('contest_id',$contest_id)->where('user_id',$group_members[$i]['user_id'])->count();
...
public function getadminrequest(){
  ...
  $user_id = Input::get('user_id');
	...
	function uninvitegroupsforcontest(){
		...
		$invited=invitegroupforcontestModel::where('group_id',$groupid)->where('contest_id',$contest_id)->where('user_id',$groupmemberid[$i]['user_id'])->count();
		if($invited!=0)
			{
				$invited=invitegroupforcontestModel::where('group_id',$groupid)->where('contest_id',$contest_id)->where('user_id',$groupmemberid[$i]['user_id'])->delete();
