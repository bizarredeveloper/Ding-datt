...
function imageIsLoaded(e) {  
	...
	function groupdelete(groupid,groupmembercnt,rowno){ 
	
	
	var defaultrowclr =$("#row_"+rowno).css("background-color");
	$("#row_"+rowno).css("background-color","#51C117");
	 var dataString="ctrlCaptionKey=alert_delete_group_msg";
				...
				success : function(data){  console.log(data);
					...
					$("#row_"+rowno).css("background-color",defaultrowclr);
				} 
				});	
...
@section('body')
    ...
    <thead>
        <tr>
			...
			<?php if(Auth::User()->ID!=1){ ?> <th class="tr_wid_button1" align="center"><span class="txt_exit">Exit</span></th><?php } ?>
	...
	if(Auth::user()->ID==1){
		...
		$group = groupModel::select('group.groupname','group.grouptype','group.createdby  as groupcreateuserid','user.firstname','user.lastname','user.username','group.groupimage','group.createdby as groupcreateuserid','group.ID as groupid','group.status')->LeftJoin('user','user.ID','=','group.createdby')->where('user.status',1)->get();
		
		}
		else{
		
		$group = groupModel::select('group.groupname','group.grouptype','group.createdby  as groupcreateuserid','user.firstname','user.lastname','user.username','group.groupimage','group.createdby as groupcreateuserid','group.ID as groupid','group.status')->where('groupname','like','%'.$searchedgroup.'%')->LeftJoin('user','user.ID','=','group.createdby')->where('user.status',1)->get();
	...
	for($i=0;$i<count($group);$i++) {
			...
			if($searchedgroup==''){ 
			$forexit = groupmemberModel::where('group_id',$group[$i]['groupid'])->where('user_id',Auth::user()->ID)->get()->count();
			}else{
			$forexit=0;
			}
        ...
        <tr id="row_{{ $i }}">
			<td>{{ $i+1; }} </td>
            <td class="tr_wid_id">{{ $group[$i]['groupname'].' ('.$groupmembercnt.')' }}</td> 
			...
			if(Auth::User()->ID==$group[$i]['groupcreateuserid']){ echo "Me";}else{
			if($group[$i]['firstname']!=''){ echo $group[$i]['firstname'].' '.$group[$i]['lastname']; }else{ echo $group[$i]['username']; } 
			}
			?></td>
            <?php if(Auth::user()->ID==1){   ?> <td><a href="#" onclick="changeactive('<?php echo $group[$i]['groupid'];?>')" <?php  if($group[$i]['status']==1){ echo 'style="background-color:green;" title="Active"'; }else{ echo 'style="background-color:red;" title="Inactive"';} ?> class="add-link backclr_<?php echo $group[$i]['groupid']; ?>"></a>
			</td> <?php } ?>
			
			<?php if(Auth::User()->ID!=1){ ?>
			<td align="center"><?php if ($forexit>0 && Auth::user()->ID != $group[$i]['groupcreateuserid']) { ?><a href="#" onClick="exitgroup('<?php echo $group[$i]['groupid']; ?>', '<?php echo Auth::user()->ID; ?>')" class="exit-link" style="background-color:red"></a><?php } ?> </td>
			<?php } ?>
			
			<td class="tr_wid_button1" align="center"><a href="<?php echo url();?>/viewgroupmember/<?php echo $group[$i]['groupid'];?>" class="view-link"title="View"></a></td>
            <td class="tr_wid_button1" align="center"><?php if(Auth::user()->ID==$group[$i]['groupcreateuserid'] ||Auth::user()->ID==1) { ?><a href="<?php echo url();?>/editgroup/<?php echo $group[$i]['groupid']; ?>" class="edit-link" title="Edit"></a><?php } ?></td>
            <td align="center"><?php if(Auth::user()->ID==$group[$i]['groupcreateuserid'] ||Auth::user()->ID==1) { ?><a  class="del-link" onclick="groupdelete('<?php echo $group[$i]['groupid'];?>','<?php echo $groupmembercnt; ?>','<?php echo $i; ?>')" title="Delete" ></a><?php } ?></td>
        </tr>
		<?php   } 
