...
<script type="text/javascript">
	...
	//// Topic photo //////////////////
		...
		$(document).on("change", ".jointopicimage", function () {
			...
			{
				$('.jointopicimage').val('');
				$(".topicimgvideoerror").html("The maximum 2MB images can be upload.");
				$("#join_con_error1").val("The maximum 2MB images can be upload.");
				return false;
			}
			else
			{
				$(".topicimgvideoerror").html("");
				$("#join_con_error1").val("");
			}
			var match = ["image/jpeg", "image/png", "image/jpg"];
			if (!((imagefile == match[0]) || (imagefile == match[1]) || (imagefile == match[2])))
			{
				$(".topicimgvideoerror").html("Upload only image file.");
				$(".topicimgvideoerror").val("Upload only image file.");
				return false;
			}
			else
			{
				$(".topicimgvideoerror").html("");
				$(".topicimgvideoerror").val("");
				var reader = new FileReader();
		...
		{
			$('.jointopicvideo').val('');
			$(".topicimgvideoerror").html("The maximum 5MB video can be upload.");
			$(".topicimgvideoerror").val("The maximum 5MB video can be upload.");
			return false;
		}
		else
		{
		...
		if (!((imagefile == "video/mp4")))
		{
			$(".topicimgvideoerror").html("Upload only video type mp4.");
			$(".topicimgvideoerror").val("Upload only video type mp4.");
			return false;
		}
		else
		{
...
if(Session::has('topicimgvideoerror')){
$topicimgvideoerror = Session::get('topicimgvideoerror');
}
...
<?php if ($tab == "invite") { ?><select class="radius sel_lang" id="subtab_mobile" onchange="subtabclick_mobile('{{$contest_id}}')">
    ...
    <div class="tab-body-wrapper">
        <div id="tab-body-1" class="tab-body">
            <div class="cont_img_con radius">
				...
				<tr><td width="150px">
					<strong><span class="pch_contestname" >Contest Name: </span></strong>
				</td>
				<td>
					<span class="mleft">
					<?php if(strlen($contestdetails['contest_name'])>20 ) echo substr(($contestdetails['contest_name']), 0, 20) . "...."; else echo $contestdetails['contest_name']; ?></div></span>
				</td></tr>
				<tr><td>
					<strong><span class="txt_contesttype" >Contest Type: </span></strong>
				</td>
				<td>
					<span class="mleft">{{ (($contestdetails['contesttype']=='p')?'Photo':(($contestdetails['contesttype']=='v')?'Video':(($contestdetails['contesttype']=='t')?'Topic':''))) }}</span>
				</td></tr>
				<tr><td>
					<strong><span class="conteststart" >Contest Start Date: </span></strong>	
				</td>
				<td>
					<span class="mleft">{{ (timezoneModel::convert($contestdetails['conteststartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span>		
				</td></tr>
				<tr><td>
					<strong><span class="contestend" >Contest End Date: </span></strong>
				</td><td>
				<span class="mleft">{{ (timezoneModel::convert($contestdetails['contestenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span>
				</td></tr>
				<tr><td>
					<strong><span class="votingstart" >Voting start Date: </span></strong>
				</td><td><span class="mleft">{{ (timezoneModel::convert($contestdetails['votingstartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></td></tr>
				<tr><td><strong><span class="votingend" >Voting End Date:</span> </strong></td>
				<td><span class="mleft">{{ (timezoneModel::convert($contestdetails['votingenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></td></tr>
				<?php if($contestdetails['prize']!='') { ?><tr><td><strong><span class="txt_prize">Prize:</span> </strong></td><td><span class="mleft">{{  $contestdetails['prize'] }}</span></td></tr><?php } ?>
				<tr><td><strong><span class="txt_noofparticipant">No. of Participants:</span> </strong></td><td><span class="mleft">{{ ($contestdetails['noofparticipant']==0)?"Unlimited":$contestdetails['noofparticipant']; }}</span></td></tr>
				</table>
				
                   <?php /* <div class="name" ><strong><span class="pch_contestname" >Contest Name: </span></strong><div class="mb_brk"></div><span class="mleft">{{ $contestdetails['contest_name'] }}</span></div>
                    ...
                    <div class="cstart"><strong><span class="conteststart" >Contest Start Date: </span></strong><div class="mb_brk"></div><span class="mleft">{{ (timezoneModel::convert($contestdetails['conteststartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></div>
                    <div class="cend"><strong><span class="contestend" >Contest End Date: </span></strong><div class="mb_brk"></div><span class="mleft">{{ (timezoneModel::convert($contestdetails['contestenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></div>
                    <div class="vstart"><strong><span class="votingstart" >Voting start Date: </span></strong><div class="mb_brk"></div><span class="mleft">{{ (timezoneModel::convert($contestdetails['votingstartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></div>
                    <div class="vend"><strong><span class="votingend" >Voting End Date:</span> </strong><div class="mb_brk"></div><span class="mleft">{{ (timezoneModel::convert($contestdetails['votingenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></div>
                   <?php if($contestdetails['prize']!='') { ?> <div class="prize"><strong><span class="txt_prize">Prize:</span> </strong><div class="mb_brk"></div><span class="mleft">{{  $contestdetails['prize'] }}</span></div><?php } ?>
        ...
        <div id="tab-body-2" class="tab-body">
            <div class="join_con" id="join_up" style="display:block">
				...
				<script>
					...
					if ($('textarea#uploadtopic').val() == ''){ 
							$("#join_con_error").html("Please enter the topic");
							return false;
					}
					}
				</script>
    ...
    ?>
								...
								<span id="topic_img_video" class="topicimgvideoerror" > {{ isset($topicimgvideoerror)?$topicimgvideoerror:'' }} </span>
								<div class="topic_cls">
                                    ...
                                    <div class="topic_imgcls">
                                        <label class="myLabel topic_img bloc">
                                            <img src="<?php if($old_cont_value['topicphoto']!=''){ echo $topicphoto; }else{ echo url().'/assets/inner/img/join_gallery.png'; } ?>" class="roundedimg roundedimgtopic">
                                            <input type="file" name="topicphoto" class="jointopicimage"  onchange="updateuploadtopicimg(this.value)" >
                                        
                                        <div class="clrsec"></div>
                                        <span class="chose_file imgphotoname">Choose your image</span></label>
                                    </div>
							<?php 
										...
										<label class="myLabel topic_img bloc">
										 ...
										 <input type="file" name="topicvideo" class="jointopicvideo" style="margin-top:22px;"   >
										 
                                        <span class="chose_file chose_vid updat_video imgvideoname">Choose your video</span>
										</label>
									  ...
									  <div >
									   <!--label class="myLabel topic_img bloc">
										...
										<!--<div class="topic_imgcls">-->
										 <label class="topic_bupload myLabel topic_img bloc">
                                    <img src="{{ URL::to('assets/inner/img/upload-vid.png') }}" class="roundedimg">	
									<div class="clrscr"></div>
									 <input type="file" name="topicvideo" class="jointopicvideo" style="margin-top:22px;"   >
									  <span class="chose_file chose_vid imgvideoname" style="top:0px; color: #313031;">Choose your video</span>
									</label>
										</div>
										 <div class="clrscr"></div>
									<?php }  ?>									
...
<script>
                        ...
                        <table class="display" cellspacing="0" width="100%" id="dd_group_list">
                            ...
                            <tbody>
                                        ...
                                        for ($i = 0; $i < $groupcount; $i++) {
                                             ...
                                             $groupmemberlistnew = groupmemberModel::where('group_id', $grouplist[$i]['groupid'])->lists('user_id');
				  						     $invitednew = invitegroupforcontestModel::whereIn('user_id', $groupmemberlistnew)->where('contest_id', $contest_id)->count();
											
											//echo count($groupmemberlistnew);
                                            $groupmemberlist_admin = groupmemberModel::where('group_id', $grouplist[$i]['groupid'])->where('user_id', 1)->get()->count(); 
											
                                            ?>
                                    <tr class="clickforrowselect" id="clickforrowselect_<?php echo $i; ?>">
                                        <td class="tr_wid_id"><input name="group_list" id="checkgrouplist_<?php echo $i; ?>" class="checkgrouplist" type="checkbox" value="{{ $grouplist[$i]['groupid'] }}"  <?php  if(count($groupmemberlistnew)==$invitednew+1) echo "checked";   ?> >
    ...
    if ($groupmemberlist_admin > 0) {
		...
		if(count($groupmemberlistnew)==$invitednew+1 && count($groupmemberlistnew)!=1){ echo "<font style='color:red;' >Invited</font>"; }else if($invitednew==0){  echo "<font style='color:green;' >Invite</font>"; }else{  echo "<font style='color:#0896D6;' >Partially Invited</font>"; }
                ...
                <form name="invite_follower" action="{{ URL::to('inviteall_follower') }}" method="post">
                    ...
                    <div class="scrolltable">
                        <table class="display" cellspacing="0" width="100%" id="dd_follower_list">
                            <thead>
                                <tr>
                                    <th style="background-color:#0896D6"><input name="" class="checkallfollowers" type="checkbox" value="1"></th>
                                    <th><span class="txt_img">Image</span></th>
...
if ($gallerytype == 1) {
                ...
                <?php echo nl2br($participants[$i]['uploadtopic']); ?>
                    ...
                    <?php if (Auth::user()->ID == 1) { ?><div class="crsl-item"><?php } ?><div class="thumbnail">
										...
										<div class="topicvideo"> <video width="80" height="80"  controls src="<?php if($participants[$i]['topicvideo']!=''){ echo url() . '/public/assets/upload/topicphotovideo/' . $participants[$i]['topicvideo']; } ?>" >
																	  ...
																	  $showtextforcontest=1;
																	}
																	//echo $noofparticipants;
                                                                }
                                                            }
    ...
    ?>
								...
								<?php 
					if($contestdetails['contesttype']=='v') $ctype= "video"; elseif($contestdetails['contesttype']=='p') $ctype= "photo"; else $ctype= "topic";
					if($showtextforcontest==1)  echo "<div class='centertext' style='margin-top:-340px;'>Voting completed for all the ".$ctype."</div>"; ?>
                            </div><!-- ib-main -->
    ...
    <?php } else if ($contestdetails[0]['contesttype'] == 'v') { ?>			
                                <video class="cont_img"   controls>											
                                    <source src="<?php echo url(); ?>/public/assets/upload/contest_participant_photo/<?php echo $participantdetails[0]['uploadfile']; ?>" type="video/mp4">								
                                </video>
    <?php } else { ?>
    ...
    ?>
                        ...
                        <a href="{{ URL::to('contesttab_close?contest_id='.$contest_id.'&contest_partipant_id='.$viewcommentforparticipant) }}" class="ib-close" >Close</a>
...
?>
                ...
                <div class="cont_img_con radius">
                    <img src="<?php echo url(); ?>/public/assets/upload/contest_theme_photo/<?php echo $contestdetails[0]['themephoto']; ?>" class="cont_img">
                     <div class="cont_list">
                       <?php /* <div class="name"><strong><span class="pch_contestname"> Contest Name: </span> </strong><div class="mb_brk"></div><span class="mleft">{{ $contestdetails[0]['contest_name']}}</span></div>
				...
				<td>
					<span class="mleft"><?php echo substr(($contestdetails[0]['contest_name']), 0, 20) . "...."; ?></div></span>
				</td></tr>
				<tr><td>
					<strong><span class="txt_contesttype" >Contest Type: </span></strong>
				</td>
				<td>
					<span class="mleft">{{ (($contestdetails[0]['contesttype']=='p')?'Photo':(($contestdetails[0]['contesttype']=='v')?'Video':(($contestdetails[0]['contesttype']=='t')?'Topic':''))) }}</span>
				</td></tr>
				<tr><td>
					<strong><span class="conteststart" >Contest Start Date: </span></strong>	
				</td>
				<td>
					<span class="mleft">{{ (timezoneModel::convert($contestdetails[0]['conteststartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span>		
				</td></tr>
				<tr><td>
					<strong><span class="contestend" >Contest End Date: </span></strong>
				</td><td>
				<span class="mleft">{{ (timezoneModel::convert($contestdetails[0]['contestenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span>
				</td></tr>
				<tr><td>
					<strong><span class="votingstart" >Voting start Date: </span></strong>
				</td><td><span class="mleft">{{ (timezoneModel::convert($contestdetails[0]['votingstartdate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></td></tr>
				<tr><td><strong><span class="votingend" >Voting End Date:</span> </strong></td>
				<td><span class="mleft">{{ (timezoneModel::convert($contestdetails[0]['votingenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span></td></tr>
				
				<tr><td><strong><span class="txt_noofparticipant">No. of Participants:</span> </strong></td><td><span class="mleft">{{ ($contestdetails[0]['noofparticipant']==0)?"Unlimited":$contestdetails[0]['noofparticipant']; }}</span></td></tr>
				</table>
				   </div> 
                </div>
...
?>
    ...
    <div class="clrscr"></div>
	<input type="hidden" name="hidadmin" id="hidadmin" value="{{ Auth::user()->ID }}" />
		...
		preloadImage(largeSrc, function() {
			...
			left	: 0
				...
				top		: 0
					...
					echo 1;
					?>';
					if (arrowshow == 1 && admin == 0)
					$this.find('div.ib-nav').show();
					if(admin == 1)
					$this.find('div.ib-nav1').show();
...
loadContentItem = function($item, callback) {
	...
	if(topicphotosrc!='') { 
	$('.topicphoto_preview').show();  
	$topicphoto_img.attr('src',topicphotosrc); }else{ 
	$('.topicphoto_preview').hide();  
	}
	if(topicvideosrc!=''){ 
	$('.topicvideo_preview').show(); 
	$topicvideo_src.attr('src',topicvideosrc); }else{ 
	$('.topicvideo_preview').hide(); 
	}
...
navigatecontent = function(dir) {
	...
	if(topicvideo!=''){ 
	$('.topic-preview-video').show();
$('.topicvideo_preview').show(); 	
	}else{ 
	$('.topic-preview-video').hide(); 
	$('.topicvideo_preview').hide(); 
	}

	if(topicphoto!=''){
	$('.topic-preview-img').show();
	$('.topicphoto_preview').show();  
	}else{
	$('.topic-preview-img').hide();
$('.topicphoto_preview').hide();  	
	}
...
jQuery('.reportshow').click(function(event){  
...
<?php if (Auth::user()->ID != 1) { ?><div class="ib-preview-flag" data-tooltip="Report Photo"  onclick="showreport('p')"  >&nbsp;</div><?php } ?>
        ...
        <div <?php /* if (Auth::user()->ID == 1) { ?> class="ib-nav1" <?php }else{   */ ?> class="ib-nav" <?php /* } */ ?> style="display:none;">				
...
<?php } ?>
    ...
    <audio id="audio1" src="<?php echo url(); ?>/assets/tones/ding.mp3"></audio>
    <audio id="audio2" src="<?php echo url(); ?>/assets/tones/err.mp3"></audio>
    <!--<div >
