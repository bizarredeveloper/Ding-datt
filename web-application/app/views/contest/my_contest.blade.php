    ...
    ?>
                            <div class="crsl-item" <?php echo ($i >= 14) ? "style='display:none'" : ""; ?> >
                                <div class="thumbnail">
                                    ...
                                    <span class="postdate">Ends on : {{ (timezoneModel::convert($photocontest[$i]['contestenddate'],'UTC',Auth::user()->timezone, $authdate)) }}</span>
                                </div>
                                <h3><a href="{{ URL::to('contest_info/'.$photocontest[$i]['ID']) }}"><?php if (strlen($photocontest[$i]['contest_name']) < 14) {
        echo $contest_name = $photocontest[$i]['contest_name'];
    } else {
        echo $contest_name = substr(($photocontest[$i]['contest_name']), 0, 14) . "...";
    } ?></a></h3>
                            </div><!-- post #1 -->
    <?php
}
...
for ($i = 0; $i < $contestcount; $i++) {
    ?>
                            <div class="crsl-item" <?php echo ($i >= 14) ? "style='display:none'" : ""; ?> >
                                ...
                                <h3><a href="{{ URL::to('edit_contest/'.$photocontest[$i]['ID']) }}"><?php if (strlen($photocontest[$i]['contest_name']) < 14) {
                            echo $contest_name = $photocontest[$i]['contest_name'];
                        } else {
                            echo $contest_name = substr(($photocontest[$i]['contest_name']), 0, 14) . "...";
                        } ?></a></h3>
