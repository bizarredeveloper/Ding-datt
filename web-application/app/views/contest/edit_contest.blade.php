...
<?php if ($noofparticipants > 0 && Auth::user()->ID != 1) {
    echo "<input type='hidden' name='contest_name' value='" . $contestdetails['contest_name'] . "'>";
						...
						<input type="hidden" name="authuserdateformat" id="authuserdateformat" value="<?php echo Auth::user()->dateformat; ?>" />
...
?>
                ...
                <div class="loginform loginbox mar2">
                    <legend class="radius"><div class="leg_head"><span id="txt_contestschedule">Contest Schedule</span></div>
                        ...
                        <input type="text" value="{{ isset($old_data['conteststartdate'])?date($authdate,strtotime($old_data['conteststartdate'])):(timezoneModel::convert($contestdetails['conteststartdate'],'UTC',Auth::user()->timezone, $authdate)) }}" <?php if (Auth::user()->ID != 1) {
    if ($noofparticipants > 0 || $contestdetails['conteststartdate'] <= date('Y-m-d H:i:s')) {
        echo "disabled";
                    ...
                    if (Auth::user()->ID != 1) {
                        ...
                        if ($noofparticipants > 0 || $contestdetails['conteststartdate'] <= date('Y-m-d H:i:s')) {
                            echo "<input type='hidden' name='conteststartdate' value='" . timezoneModel::convert($contestdetails['conteststartdate'], 'UTC', Auth::user()->timezone, $authdate) . "'>";
                        }
                    }
                    ?>
                        </p>
                        ...
                        <p>
                        <div class="inp_pfix"><img src="{{ URL::to($assets_path.'img/date_icons.png')}}" width="25" height="25"></div>
                        <input type="text" id="contestend" name="contestenddate" placeholder="Contest End Date" title="Contest End Date" value="{{ isset($old_data['contestenddate'])?date($authdate,strtotime($old_data['contestenddate'])):(timezoneModel::convert($contestdetails['contestenddate'],'UTC',Auth::user()->timezone, $authdate)) }}" <?php if (Auth::user()->ID != 1) {
...
<?php if (Auth::user()->ID != 1) {
    if ($contestdetails['votingstartdate'] <= date('Y-m-d H:i:s') || $contestdetails['contestenddate'] <= date('Y-m-d H:i:s')) {
        echo "<input type='hidden' name='contestenddate' value='" . timezoneModel::convert($contestdetails['contestenddate'], 'UTC', Auth::user()->timezone, $authdate) . "'>";
    }
} ?>
                        </p>
                    ...
                    <legend class="radius"><div class="leg_head"><span id="txt_votingschedule">Voting Schedule</span></div>
                        <p>
                        <div class="inp_pfix"><img src="{{ URL::to($assets_path.'img/date_icons.png')}}" width="25" height="25"></div>
                        <input type="text" id="votingstart" name="votingstartdate" placeholder="Voting Start Date" title="Voting Start Date" value="{{ isset($old_data['votingstartdate'])?date($authdate,strtotime($old_data['votingstartdate'])):(timezoneModel::convert($contestdetails['votingstartdate'],'UTC',Auth::user()->timezone, $authdate)) }}" <?php if (Auth::user()->ID != 1) {
...
<?php if (Auth::user()->ID != 1) {
    if ($contestdetails['votingstartdate'] <= date('Y-m-d H:i:s')) {
        echo "<input type='hidden' name='votingstartdate' value='" . timezoneModel::convert($contestdetails['votingstartdate'], 'UTC', Auth::user()->timezone, $authdate) . "'>";
    }
} ?>
                        </p>
                        ...
                        <p>
                        <div class="inp_pfix"><img src="{{ URL::to($assets_path.'img/date_icons.png')}}" width="25" height="25"></div>
                        <input type="text" id="votingend" name="votingenddate" placeholder="Voting End Date" title="Voting End Date" value="{{ isset($old_data['votingenddate'])?date($authdate,strtotime($old_data['votingenddate'])):(timezoneModel::convert($contestdetails['votingenddate'],'UTC',Auth::user()->timezone, $authdate)) }}" <?php if (Auth::user()->ID != 1) {
...
<?php if (Auth::user()->ID != 1) {
    if ($contestdetails['votingenddate'] <= date('Y-m-d H:i:s') || $contestdetails['votingenddate'] <= date('Y-m-d H:i:s')) {
        echo "<input type='hidden' name='votingenddate' value='" . timezoneModel::convert($contestdetails['votingenddate'], 'UTC', Auth::user()->timezone, $authdate) . "'>";
    }
} ?>
                        </p>                
                    </legend>
<?php if (Auth::User()->ID == 1) { ?>
                        <legend class="radius" style="height:230px"><div class="leg_head"><span id="txt_sponsorinfo">Sponsor Info</span></div>
                            ...
                            <div class="img_pfix mb_upimg"><img src="<?php if($contestdetails['sponsorphoto']!=''){ echo $sponsorpath.$contestdetails['sponsorphoto']; }else{ echo url().'/assets/inner/images/avator.png'; } ?> " width="45" height="45" class="roundedimg roundedimgsopn"></div> 
                            <p>
                                <label class="myLabel">
