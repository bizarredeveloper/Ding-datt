...
function deletecategory(categoryid,rowno) {
...
if(Session::has('er_data_create')){
	$er_data_create = Session::get('er_data_create');
}
                    ...
                    <tbody>
                        ...
                        for ($i = 0; $i < count($categorydetails); $i++) {
                            ...
                            <tr id="row_{{ $i }}">
                                ...
                                <td><a href="<?php echo url() . '/gotoeditcategory?interestid=' . $categorydetails[$i]['Interest_id']; ?>"  class="edit-link" title="Edit"></a></td>
                                <td><a href="#" onclick="deletecategory('<?php echo $categorydetails[$i]['Interest_id']; ?>','<?php echo $i; ?>')"  class="del-link" title="Delete" ></a></td>
                            </tr>
                        <?php }
                        ?>
                    </tbody>
                    ...
                    ?><form id="editprofile" name="group" enctype="multipart/form-data" action="addcategory" method="post" class="form_mid"><?php } ?>
                        ...
                        <div class="loginform loginbox grp_ctr"><!-- mar1-->
                            <legend class="radius" style="min-height:100px;"><div class="leg_head">Category details</div>
                                ...
                                @if(isset($er_data_create))
                                <p class="alert" style="color:green;padding:5px;text-align:center;font-size:13px">{{ $er_data_create }}</p>
                                @endif
