	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			toggleCol: function(cid,visible) {
				...
				if ($('input.chk_colunm:checked',g.nDiv).length<p.minColToggle&&!visible) return false;
			...
			switchCol: function(cdrag,cdrop) { //switch columns
				...
				$('tbody tr',t).each
					...
					if ($.browser.msie&&$.browser.version<7.0) $('tr:eq('+cdrop+') input.chk_colunm',this.nDiv)[0].checked = true;	
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='Clear' /> <input type='button' id='jsearch_btn' value='Search'  /> </div>");
                $('#jsearch_btn',g.sDiv).click(function(e){ get_bottom_Search(); });
				//g.doSearch();
				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13)   get_bottom_Search(); });
				$('input[value=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = '';  get_bottom_Search(); });
				$(g.bDiv).after(g.sDiv);				
		...
		*/}
		...
		if ($('th',g.hDiv).length)
		{
			 	...
			 	function ()
					{
						...
						//$('tbody',g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol chk_colunm" value="'+ cn +'"  /></td><td class="ndcol2">'+this.innerHTML+'</td></tr>');
						
						if( kcol.title){}
else { $('tbody', g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" ' + chk + ' class="togCol chk_colunm" value="' + cn + '" /></td><td class="ndcol2">' + this.innerHTML + '</td></tr>'); }
						cn++;
					}
			);
			...
			(
			 	function ()
					{
						if ($('input.chk_colunm:checked',g.nDiv).length<=p.minColToggle&&$(this).prev().find('input')[0].checked) return false;
						return g.toggleCol($(this).prev().find('input.chk_colunm').val());
					}
			);
			 	...
			 	function ()
					{
						...
						if ($('input.chk_colunm:checked',g.nDiv).length<p.minColToggle&&this.checked==false) return false;
